    ...
    try {
        ...
        if (cData && cData.data && Array.isArray(cData.data.channels)) {
            ...
            flatChannels.forEach(channel => {
                let firstGenre = channel.languages && channel.languages.length > 0 ? channel.languages[0] : null;
...
const generateM3u = async (ud) => {
  ...
  for (let i = 0; i < chansList.length; i++) {
    ...
    m3uStr += 'group-title="NBKTSK-' + (chansList[i].group_title) + '", tvg-logo="https://mediaready.videoready.tv/tatasky-epg/image/fetch/f_auto,fl_lossy,q_auto,h_250,w_250/' + (chansList[i].tvg_logo) + '", ' + chansList[i].name + '\n';
